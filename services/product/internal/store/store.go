...
func (s *Store) ListProductsFiltered(
	ctx context.Context,
	...
	if v := filters["q"]; v != "" {
		query += fmt.Sprintf(
			" AND (p.title ILIKE $%d OR p.short_desc ILIKE $%d)",
			argIndex, argIndex,
		)
		args = append(args, "%"+v+"%")
		argIndex++
...
func (s *Store) HasMediaOrder(
	...
	err := s.db.QueryRow(ctx, `
		SELECT EXISTS (
			...
			FROM product_media
			WHERE product_id = $1
			  AND (meta->>'order')::int = $2
		)
	`, productID, order).Scan(&exists)
