...
// ✅ Fix TypeScript error for window.Razorpay
declare global {
  interface Window {
    Razorpay: any;
  }
...
export default function CheckoutPage() {
  ...
  // --- 1. RAZORPAY MODAL HANDLER ---
  const openRazorpay = (orderData: any, paymentData: any) => {
    const options = {
      ...
      name: "Sabhyatam",
      description: `Order #${orderData.order_id.slice(0, 8)}`,
      order_id: paymentData.gateway_order_id, // The 'order_...' ID from Backend
      
      // ✅ SUCCESS HANDLER
      handler: async function (response: any) {
        try {
          // Verify Signature with Backend
          const res = await fetch('http://localhost:8083/v1/payments/verify', {
            method: 'POST',
      ...
      modal: {
        ondismiss: function() {
          setPaying(false) // Re-enable button if user closes modal
        }
      }
    };
  ...
  async function handlePay() {
    try {
      ...
      // B. INITIATE PAYMENT (Payments Service: Port 8083)
      // Calls CreatePaymentOrder on Backend -> Returns Razorpay Order ID
      const payRes = await fetch(
        'http://localhost:8083/v1/payments/initiate', 
        {
          method: 'POST',
          headers: { 
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`,
            'X-USER-ID': user.id,
            'X-ORDER-ID': order.order_id, // Important for backend mapping
      ...
      if (!payRes.ok) {
        ...
        throw new Error(errJson.error || 'Payment initiation failed')
      }

      const paymentData = await payRes.json()

      // C. OPEN REAL RAZORPAY MODAL
      openRazorpay(order, paymentData)
